...
export default function MapView() {
  ...
  const [selectedTypes, setSelectedTypes] = useState<string[]>(crimeTypes.map((type) => type.id))
  ...
  const fetchIncidents = async (lat: number, lng: number, radius: number) => {
    ...
    try {
      ...
      const nearbyResponse = await fetch(`/api/incidents?lat=${lat}&lng=${lng}&radius=${radius}`)
  ...
  const filteredIncidents = incidents.filter((incident) => selectedTypes.includes(incident.type))
    ...
    <div className="flex flex-col md:flex-row h-screen w-full">
      ...
      <div className="w-full md:w-80 bg-background border-r p-4 overflow-y-auto">
                ...
                fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(searchInput)}`)
                  ...
                  .then((data) => {
                    if (data && data.length > 0) {
                      ...
                      fetchIncidents(Number.parseFloat(lat), Number.parseFloat(lon), radius)
                    }
                  })
        ...
        <div className="mb-6">
          ...
          <Slider value={[radius]} min={500} max={5000} step={500} onValueChange={(value) => setRadius(value[0])} />
      ...
      <div className="flex-1 relative h-screen">
        ...
        <MapContainer center={mapCenter} zoom={13} style={{ height: "100%", width: "100%" }} zoomControl={false}>
          <MapController center={mapCenter} zoom={13} selectedIncident={selectedIncident} incidents={allIncidents} />
          <TileLayer
          ...
          {allIncidents
            ...
            .map((incident) => (
              <Marker
                ...
                icon={L.divIcon({
                  className: `custom-div-icon ${selectedIncident === incident.id ? "selected-marker" : ""}`,
                  iconSize: [30, 30],
                  iconAnchor: [15, 15],
                  html: `<div style="background-color: ${
                    crimeTypes.find((t) => t.id === incident.type)?.color || "#581845"
                  };" class="marker-pin ${selectedIncident === incident.id ? "selected-marker-pin" : ""}"></div>`,
                })}
              >
                <Popup>
                  <div className="p-2">
                  ...
                  >
                    ...
                    <CardFooter className="p-4 pt-2 flex justify-between items-center">
                      <Badge
                        variant="outline"
                        style={{
                          backgroundColor: crimeTypes.find((t) => t.id === incident.type)?.color + "20",
